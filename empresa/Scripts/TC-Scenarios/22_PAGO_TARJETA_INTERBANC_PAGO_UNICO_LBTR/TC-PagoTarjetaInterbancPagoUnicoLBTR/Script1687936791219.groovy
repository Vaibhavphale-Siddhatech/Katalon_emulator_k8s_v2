...
'PAGO TARJETA INTERBANC PAGO UNICO LBTR : '
...
if (Mobile.waitForElementPresent(findTestObject('PagosYTransferencia/Origen-Title'), 10, FailureHandling.OPTIONAL)) {
	...
	WebUI.callTestCase(findTestCase('CommonTestCases/TC_DestinoAccountPagoUnico'), [('CtaDestino') : CtaDestino, ('Banco') : Banco, ('Moneda') : Moneda, ('TipoDeProducto') : TipoDeProducto, ('CuentaDestino') : CuentaDestino, ('Description') : Description, ('TipoDeDocumento') : TipoDeDocumento, ('NumeroDeDocumento') : NumeroDeDocumento], FailureHandling.STOP_ON_FAILURE)
	
    'ENTER MONTO FOR PAYMENT :'
	 println "Monto For Transaction Is : " + Monto
	 WebUI.callTestCase(findTestCase('CommonTestCases/TC_Monto'), [('Monto') : Monto], FailureHandling.STOP_ON_FAILURE)

	'TASA VERIFICATION : '
	 WebUI.callTestCase(findTestCase('CommonTestCases/TC_GetTasa'), [:], FailureHandling.OPTIONAL)
 
	'ENTER PAGO TYPE ACH/LBTR FOR PAYMENT :'
	WebUI.callTestCase(findTestCase('CommonTestCases/TC_PagoMethod'), [('PagoType') : PagoType], FailureHandling.STOP_ON_FAILURE)
