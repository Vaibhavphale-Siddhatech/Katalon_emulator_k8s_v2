...
def checkCurrency() {
	try {
		...
		double UpdatedMonto = Double.parseDouble(GlobalVariable.UpdatedMonto)
		if(UpdatedMonto > Monto) {
			GlobalVariable.CurrencyTypeOrigen = 'DOP'
		}
	} catch (Exception e) {
		if(GlobalVariable.CurrencyTypeOrigen != 'DOP') {
			GlobalVariable.CurrencyTypeOrigen = 'DOP'
		}
	}
}