...
WebUI.callTestCase(findTestCase('CommonTestCases/TC_ProfileVerification'), [('Verification') : Verification, ('Description') : Description,('CuentaOrigen') : CuentaOrigen,('CuentaDestino') : CuentaDestino,
	('Impuesto'):Impuesto,('Monto'):Monto,('TransactionMessage'):TransactionMessage,('TransactionMessageAuthorizer1'):TransactionMessageAuthorizer1,('TransactionMessageAuthorizer2'):TransactionMessageAuthorizer2, ('MontoCurrency'):MontoCurrency], FailureHandling.STOP_ON_FAILURE)
...
if (Mobile.waitForElementPresent(findTestObject('PagosYTransferencia/Origen-Title'), 5, FailureHandling.OPTIONAL)) {
	...
	WebUI.callTestCase(findTestCase('CommonTestCases/TC_Monto - Tarjeta'), [('Monto') : Monto, ('MontoCurrency') : MontoCurrency, ('MontoType') : MontoType], FailureHandling.STOP_ON_FAILURE)

	'REPLACE MONTO VALUE : '
	GlobalVariable.montoReplace = MontoCurrency
	String checkMonto = GlobalVariable.OriginalMonto
	if(checkMonto.matches(".*[a-zA-Z]+.*")) {
		String preDefinedMonto = Mobile.getText(findTestObject('TarjetaPropia/Monto on datos screen'), 1, FailureHandling.OPTIONAL)
		String Balance1 = preDefinedMonto.replaceAll('[,]', '')
		String Balance2 = Balance1.substring(4)
		GlobalVariable.OriginalMonto = Balance2
		if(MontoCurrency == "DOP") {
			Monto = GlobalVariable.OriginalMonto
		} else {
			Monto = Double.toString((Double.parseDouble(GlobalVariable.OriginalMonto)*GlobalVariable.USD_to_DOP_rate));
		}	
	} else {
		if(MontoCurrency == "USD") {
			Monto = Double.toString((Double.parseDouble(GlobalVariable.OriginalMonto)*GlobalVariable.USD_to_DOP_rate));
		}
	}
	...
	if (Concept == '') {
		...
		Mobile.setText(findTestObject('Interbancario_Beneficiary/android.widget.EditText - Digita un concepto'),Concept, 5)
	...
	'CHECK FOR NO MONTO AVAILABLE : '
	if(Mobile.verifyElementVisible(findTestObject('PaymentMessages/Por favor complete el campo pago mnimo'), 2, FailureHandling.OPTIONAL)) {
		GlobalVariable.PaymentMessage = Mobile.getText(findTestObject('PaymentMessages/Por favor complete el campo pago mnimo'),3, FailureHandling.OPTIONAL)
		GlobalVariable.AppPaymentMessage = GlobalVariable.PaymentMessage
		GlobalVariable.CaseStatusMessage = "*** Pago Minimo is not available ***"
		CustomKeywords.'helper.customfunction.Screenshot'()
		KeywordUtil.markFailedAndStop("Pago Minimo is not available")
	} else if(Mobile.verifyElementVisible(findTestObject('PaymentMessages/Por favor complete el campo balance a la fecha'), 2, FailureHandling.OPTIONAL)) {
		GlobalVariable.PaymentMessage = Mobile.getText(findTestObject('PaymentMessages/Por favor complete el campo balance a la fecha'),3, FailureHandling.OPTIONAL)
		GlobalVariable.AppPaymentMessage = GlobalVariable.PaymentMessage
		GlobalVariable.CaseStatusMessage = "*** Balance a la fecha is not available ***"
		CustomKeywords.'helper.customfunction.Screenshot'()
		KeywordUtil.markFailedAndStop("Balance a la fecha is not available")
	} else if(Mobile.verifyElementVisible(findTestObject('PaymentMessages/Por favor complete el campo balance al corte'), 2, FailureHandling.OPTIONAL)) {
		GlobalVariable.PaymentMessage = Mobile.getText(findTestObject('PaymentMessages/Por favor complete el campo balance al corte'),3, FailureHandling.OPTIONAL)
		GlobalVariable.AppPaymentMessage = GlobalVariable.PaymentMessage
		GlobalVariable.CaseStatusMessage = "*** Balance al corte is not available ***"
		CustomKeywords.'helper.customfunction.Screenshot'()
		KeywordUtil.markFailedAndStop("Balance al corte is not available")
	}
	
	 'VERIFICATION CODIGO : '
	 WebUI.callTestCase(findTestCase('CommonTestCases/TC_CodigoVerification'), [('Monto') : Monto, ('Password') : Password, ('Description') : Description], FailureHandling.STOP_ON_FAILURE)
...
def PaymentVerification() {
		...
		'VERIFY THE BALANCES : '
		WebUI.callTestCase(findTestCase('CommonTestCases/TC_VerifyResults'), [('Verification') : Verification,('Description') : Description,('CuentaOrigen') : CuentaOrigen, ('CuentaDestino') : CuentaDestino,('CtaOrigen') : CtaOrigen, ('CtaDestino') : CtaDestino,('Monto') : Monto,('Impuesto') : Impuesto, ('MontoCurrency') : MontoCurrency], FailureHandling.OPTIONAL)
