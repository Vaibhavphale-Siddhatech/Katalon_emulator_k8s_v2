...
if(Concept == "SwipeVerification") {
	...
	if (Mobile.waitForElementPresent(findTestObject('PagosYTransferencia/Origen-Title'), 5, FailureHandling.OPTIONAL)) {
		...
		if (Concept == 'OriginConsulatationPermissions') {
			Mobile.tap(findTestObject('PagosYTransferencia/Destino-Title'), 5, FailureHandling.OPTIONAL)
			Mobile.delay(1)
			Mobile.waitForElementPresent(findTestObject('PagosYTransferencia/Cuentas-List'), 05, FailureHandling.OPTIONAL)
            println('Concept is origin Consulatation Permissions' + Description)
			try {
				...
				if(Element.displayed.TRUE) {
					KeywordUtil.markFailed('Destino account is available for restricted account')
				}
			} catch(Exception e){
				KeywordUtil.markPassed('Destino acccount is not available for restricted account')
			}
          } else {  
